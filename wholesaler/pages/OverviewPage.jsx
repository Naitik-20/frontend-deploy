      ...
      >
        ...
        <div className="profile-summary-body">
          <div className="profile-photo-wrap">
            ...
            <label className="photo-camera-btn" aria-label="Upload profile image" onClick={(event) => event.stopPropagation()}>
              <Camera size={14} />
              <input type="file" accept="image/*" onChange={onImageUpload} />
            </label>
          </div>
          <div className="profile-summary-copy">
            <strong>{profile?.name || 'Wholesaler'}</strong>
            <span>{profile?.phone || 'Phone not added'}</span>
            <p>{profile?.shopName || 'Shop name not added'}</p>
          </div>
          ...
          <div className="orders-list recent-orders-list">
            {recentOrders.map((order) => (
              <Link
                ...
                state={{ order }}
                className="order-row recent-order-row"
                key={order._id}
              >
                <div>
